function report = inspect_per_scenario_Ht_exact_overlap(saveRoot, scenarioTag)
%INSPECT_PER_SCENARIO_HT_EXACT_OVERLAP
% Check exact Ht duplicates within one per-scenario dataset.
%
% Example:
%   report = inspect_per_scenario_Ht_exact_overlap( ...
%       'dataset_E2E4_mix', 'E4_IndoorHall_5GHz');
%
% Inputs:
%   saveRoot    : root directory containing per_scenario/
%   scenarioTag : e.g.
%       'E2_CDL-B'
%       'E2_CDL-E'
%       'E4_IndoorHall_5GHz'
...
%   report.cross.train_vs_valid / train_vs_test / valid_vs_test
    ...
    if nargin < 2 || isempty(scenarioTag)
        error('scenarioTag is required.');
    end
    ...
    report = struct();
    report.scenarioTag = scenarioTag;
    report.self = struct();
    report.cross = struct();

    fprintf('\n==================================================\n');
    fprintf('SELF exact-Ht check (per-scenario)\n');
    fprintf('==================================================\n');
    ...
    fprintf('\n==================================================\n');
    fprintf('CROSS exact-Ht check (per-scenario)\n');
    fprintf('==================================================\n');
    ...
    for i = 1:numel(splitNames)
        sname = splitNames{i};
        r = report.self.(sname);
        fprintf('[SELF | %s] exact pairs = %d, candidate pairs = %d\n', ...
            sname, r.nExactPairs, r.nCandidatePairs);
    end

    keys = fieldnames(report.cross);
    for i = 1:numel(keys)
        r = report.cross.(keys{i});
        fprintf('[CROSS | %s] exact pairs = %d, candidate pairs = %d\n', ...
            keys{i}, r.nExactPairs, r.nCandidatePairs);
    end
end
...
%   - among those candidates, test exact Ht equality
    ...
    if rep.nExactPairs > 0
        ...
        runs = find_pair_runs(rep.exactPairs);
        rep.runs = runs;
        if ~isempty(runs)
...
% Across two splits:
%   - intersect overlapping gain values
...
% ============================================================
function runs = find_pair_runs(pairs)
% Detect simple run-like structure in exact-pair list [i,j]
% Returns [start_i, end_i, start_j, end_j, len]
